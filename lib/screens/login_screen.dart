...
import 'package:remind_me/util/getLogin_details.dart';
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  User user = User();
  _setCredentials(Map<String, dynamic> value) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    prefs.setStringList(
        "user", [value['id'], value['type'], value['name'], value['email']]);
    prefs.setBool("session", true);
  }

  void validate() {
    if (formkey.currentState.validate()) {
      logIn(user.uEmail.trim(), user.uPassword.trim()).then((userId) {
        if (userId != "error") {
          getLoginCredentials(userId).then((data) {
